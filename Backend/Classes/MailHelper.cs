    ...
    public class MailHelper
    {
        public static async Task SendMail(string to, string subject, string body)
        {
            ...
            using (var smtp = new SmtpClient())
            {
                ...
                smtp.EnableSsl = true; await smtp.SendMailAsync(message);
            }
        }
...

        {
            ...
            message.From = new MailAddress(WebConfigurationManager.AppSettings["AdminUser"]);
            message.Subject = subject; message.Body = body;
            message.IsBodyHtml = true; using (var smtp = new SmtpClient())
            {
                var credential = new NetworkCredential
                {
                    UserName = WebConfigurationManager.AppSettings["AdminUser"],
                ...
                smtp.Credentials = credential; smtp.Host = WebConfigurationManager.AppSettings["SMTPName"];
                smtp.Port = int.Parse(WebConfigurationManager.AppSettings["SMTPPort"]);
                smtp.EnableSsl = true; await smtp.SendMailAsync(message);
            }
        }
